...
public class APIGetTunnelLog implements HttpHandler {
    @Override
    public void handle(HttpExchange exchange) throws IOException {
        ...
        try {
            ...
            if (!CacheProcess.processMap.containsKey(id)) {
                responseData.replace("message","没有这个隧道哦！");
                return;
            }
            //写入响应数据
            responseData.replace("log",CacheProcess.processMap.get(id).logBuffer.toString());
